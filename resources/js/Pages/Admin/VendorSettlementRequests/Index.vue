    ...
    <AuthenticatedLayout>
        ...
        <section class="section">
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">{{ $t('Vendor Settlement Requests') }}</h5>
                            ...
                            <div v-if="!requests.data || requests.data.length === 0" class="text-center py-12">
                                ...
                                <p class="text-gray-500">{{ $t('No settlement requests found.') }}</p>
                            </div>
        ...
        <div class="modal fade" id="rejectModal" tabindex="-1" aria-labelledby="rejectModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="rejectModalLabel">{{ $t('Reject Settlement Request') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="alert alert-warning">
                            <i class="bi bi-exclamation-triangle me-2"></i>
                            {{ $t('You are about to reject this settlement request. Please provide a reason.') }}
                        </div>
                        
                        <div v-if="currentRequest" class="mb-3">
                            <strong>{{ $t('Vendor') }}:</strong> {{ currentRequest.vendor_name }}<br>
...
const approveRequest = (requestId) => {
    if (confirm(t('Are you sure you want to approve this settlement request?'))) {
        router.post(route('admin.settlement-requests-vendor.approve', requestId), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
const confirmReject = () => {
    ...
    router.put(route('admin.settlement-requests-vendor.reject', currentRequest.value.id), {
